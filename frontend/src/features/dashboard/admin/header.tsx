...
import { useBranchScope } from '@/shared/hooks/use-branch-scope';
...
export function AdminHeader({ onMenuToggle }: AdminHeaderProps) {
  ...
  return (
    <header className="flex items-center justify-between gap-4 bg-white border-b border-border px-6 py-4 shadow-sm shrink-0 relative z-30">
      {/* Left: Hamburger + Branch Switcher (ADM-002) */}
      <div className="flex items-center gap-4">
        <button
          onClick={onMenuToggle}
          title="Toggle Sidebar Menu"
          className="flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted transition-colors active:scale-95"
        >
          <Menu size={20} />
        </button>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-border bg-[#FAF6F0] px-4 py-2 text-sm font-semibold text-gray-700 hover:border-[#BA935D] hover:text-[#BA935D] transition-all"
          >
          ...
          {isDropdownOpen && (
            <div className="absolute left-0 top-full mt-2 w-64 rounded-xl border border-border bg-white shadow-lg z-50 py-1.5 animate-fadeIn">
              <div className="px-3 py-1.5 border-b border-gray-100 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">
              ...
              {displayList.map((b) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm transition-colors flex items-center justify-between ${
                    currentBranch?.id === b.id ? 'bg-[#BA935D]/10 text-[#BA935D] font-semibold' : 'text-gray-700 hover:bg-muted'
                  }`}
                >
                  <span>{b.name}</span>
      ...
      <div className="hidden md:flex flex-1 max-w-md">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border bg-muted py-2.5 pl-9 pr-4 text-sm text-gray-700 placeholder:text-muted-foreground focus:border-[#BA935D] focus:outline-none focus:ring-1 focus:ring-[#BA935D] transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        {/* Notification Bell */}
        <button className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-border text-muted-foreground hover:border-[#BA935D] hover:text-[#BA935D] transition-all">
          <Bell size={18} />
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2.5 rounded-xl border border-border bg-[#FAF6F0] px-3 py-2 text-sm font-semibold text-gray-700 hover:border-[#BA935D] transition-all"
          >
          ...
          {/* User Dropdown Box */}
          {showUserMenu && (
            <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-border bg-white p-3 shadow-xl z-50 animate-fadeIn">
              ...
              <button
                ...
                className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-muted transition-colors"
              >
                <UserCheck size={14} className="text-muted-foreground" /> Profil & Sesi
              </button>
              <button

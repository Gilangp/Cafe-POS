              ...
              {(['pickup', 'dinein', 'delivery'] as const).map((mode) => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-bold transition-all ${
                    orderType === mode
                      ? 'bg-[#BA935D] text-[#12100E]'
                      : 'text-white/80 hover:text-white'
                  }`}
                >
                  {mode === 'pickup' ? 'Ambil (Pickup)' : mode === 'dinein' ? 'Dine-In Meja' : 'Kurir Delivery'}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="relative w-full max-w-lg rounded-3xl bg-white shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            ...
            <div className="border-t border-gray-100 p-6 bg-gray-50 flex gap-3">
              ...
              >
                <Plus size={16} />
                <span>Masuk ke Keranjang · {fmt(selectedProduct.price + (milkOption.includes('Oat Milk') || milkOption.includes('Almond Milk') ? 10000 : 0))}</span>
              </button>
            </div>
          </div>
        </div>
              ...
              {checkoutStep === 'success' ? (
                ...
                <div className="space-y-6">
                  <div className="rounded-2xl bg-[#FAF6F0] p-4 border border-gray-200 space-y-2">
                    <p className="text-xs font-bold uppercase tracking-wider text-[#BA935D]">Ringkasan Pengiriman / Pengambilan</p>
                    <p className="text-sm font-bold text-gray-800">{branch}</p>
                    <p className="text-xs text-gray-500 capitalize">Mode: {orderType === 'pickup' ? 'Ambil di Toko' : orderType === 'dinein' ? 'Makan di Meja' : 'Kurir Delivery'}</p>
                  </div>

                  <div className="space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-gray-400">Detail Pembayaran</h4>
                    <div className="space-y-2 text-sm text-gray-600">
                      <div className="flex justify-between"><span>Subtotal Item</span><span>{fmt(subtotal)}</span></div>
                      {appliedDiscount > 0 && <div className="flex justify-between text-green-600 font-semibold"><span>Diskon Promo</span><span>-{fmt(appliedDiscount)}</span></div>}
                      <div className="flex justify-between"><span>PPN 11%</span><span>{fmt(tax)}</span></div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {['QRIS (GoPay/OVO/Dana)', 'Kartu Debit/Kredit', 'Pay at Counter'].map((m, i) => (
                        <label key={m} className={`flex items-center gap-2 rounded-xl border p-3 text-xs font-semibold cursor-pointer ${i === 0 ? 'border-[#BA935D] bg-[#BA935D]/10 text-[#BA935D]' : 'border-gray-200 text-gray-700'}`}>
                ...
                <div className="flex flex-col items-center justify-center h-full text-center text-gray-400 py-20">
                  ...
                  <p className="text-sm font-medium">Keranjang Anda masih kosong.<br />Yuk, pilih kopi favoritmu di katalog!</p>
                </div>
              ) : (
                  ...
                  {cart.map((item) => (
                    <div key={item.id} className="flex items-start gap-3 rounded-2xl border border-gray-100 p-3.5 bg-gray-50/50">
                      ...
                      <div className="flex items-center gap-1">
                        <button onClick={() => updateCartQty(item.id, -1)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-gray-200 text-gray-600 hover:bg-red-100 hover:text-red-600 transition-colors">
                          <Minus size={12} />
                        </button>
                        <span className="w-5 text-center text-xs font-bold text-gray-800">{item.qty}</span>
                        <button onClick={() => updateCartQty(item.id, 1)} className="flex h-7 w-7 items-center justify-center rounded-lg bg-[#BA935D] text-white hover:bg-[#a07d4e] transition-colors">
                          <Plus size={12} />
                        </button>
                      </div>
                    </div>
            ...
            {cart.length > 0 && checkoutStep !== 'success' && (
              <div className="border-t border-gray-100 p-6 bg-gray-50 space-y-4">
                <div className="space-y-1 text-sm">
                  ...
                  {appliedDiscount > 0 && <div className="flex justify-between text-green-600 text-xs font-bold"><span>Diskon Promo</span><span>-{fmt(appliedDiscount)}</span></div>}
                  <div className="flex justify-between text-gray-500 text-xs"><span>PPN 11%</span><span>{fmt(tax)}</span></div>
